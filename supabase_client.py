...
def upsert_races(races_data: list[dict], retry=True):
    ...
    try:
        return supabase.table("races").upsert(races_data, on_conflict="race_date,place_code,race_number", returning="minimal").execute()
    except Exception as e:
...
def upsert_entries(entries_data: list[dict], retry=True):
    ...
    supabase = get_supabase_client()
    try:
        return supabase.table("entries").upsert(entries_data, on_conflict="race_date,place_code,race_number,boat_number", returning="minimal").execute()
    except Exception as e:
        if retry and ("disconnected" in str(e).lower() or "connection" in str(e).lower()):
            get_supabase_client(force_refresh=True)
...
def upsert_favorites(favorites_data: list[dict], retry=True):
    ...
    supabase = get_supabase_client()
    try:
        return supabase.table("favorite_racers").upsert(favorites_data, on_conflict="toban", returning="minimal").execute()
    except Exception as e:
        if retry and ("disconnected" in str(e).lower() or "connection" in str(e).lower()):
            get_supabase_client(force_refresh=True)
...
def cleanup_supabase_storage(threshold_date_iso: str, retry=True):
    ...
    try:
        ...
        }, returning="minimal").lt("race_date", threshold_date_iso).execute()
...
def delete_very_old_races(threshold_date_iso: str, retry=True):
    ...
    try:
        ...
        supabase.table("entries").delete(returning="minimal").lt("race_date", threshold_date_iso).execute()
        
        # 2. レース本体の削除 (races)
        return supabase.table("races").delete(returning="minimal").lt("race_date", threshold_date_iso).execute()
    except Exception as e:
        if retry and ("disconnected" in str(e).lower() or "connection" in str(e).lower()):
            get_supabase_client(force_refresh=True)
